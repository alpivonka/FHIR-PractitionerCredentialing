...
const FSHVisitor_1 = __importDefault(require("./generated/FSHVisitor"));
const FSHLexer_1 = __importDefault(require("./generated/FSHLexer"));
const FSHParser_1 = __importDefault(require("./generated/FSHParser"));
...
const MiniFSHImporter_1 = require("./MiniFSHImporter");
var SdMetadataKey;
...
function unescapeUnicode(match, p1) {
 ...
 */
class FSHImporter extends FSHVisitor_1.default {
    constructor() {
        super();
        this.docs = [];
    ...
    visitExtension(ctx) {
        ...
        if (this.docs.some(doc => doc.extensions.has(extension.name))) {
            ...
            ctx.context().forEach(extContext => {
                var _a;
                if (((_a = extension.contexts) === null || _a === void 0 ? void 0 : _a.length) > 0) {
                    FSHLogger_1.logger.error("Metadata field 'Context' already declared.", {
                        file: this.currentFile,
    ...
    visitLogical(ctx) {
        ...
        if (this.docs.some(doc => doc.logicals.has(logical.name))) {
            ...
            ctx.characteristics().forEach(characteristics => {
                var _a;
                if (((_a = logical.characteristics) === null || _a === void 0 ? void 0 : _a.length) > 0) {
                    FSHLogger_1.logger.error("Metadata field 'Characteristics' already declared.", {
                        file: this.currentFile,
            ...
            if (rule instanceof rules_1.ValueSetConceptComponentRule) {
                const matchedComponent = valueSet.rules.find(existingComponent => {
                    return (existingComponent instanceof rules_1.ValueSetConceptComponentRule &&
                        ...
                        (0, isEqual_1.default)((0, sortBy_1.default)(rule.from.valueSets), (0, sortBy_1.default)(existingComponent.from.valueSets)));
                });
                if (matchedComponent) {
    ...
    parseInvariant(invariant, metaCtx = [], ruleCtx = []) {
        const seenPairs = new Map();
        metaCtx
    ...
    visitParamRuleSet(ctx) {
        ...
        const paramRuleSet = new fshtypes_1.ParamRuleSet(name)
            .withLocation(this.extractStartStop(ctx))
            .withFile(this.currentFile);
    ...
    visitUsage(ctx) {
        ...
        let usage = (0, upperFirst_1.default)(usageConcept.code);
        if (!(0, fshtypes_1.isInstanceUsage)(usage)) {
    ...
    visitContext(ctx) {
        ...
        ctx.contextItem().forEach(contextItem => {
            if (contextItem.QUOTED_CONTEXT()) {
                contexts.push({
                    value: this.unescapeQuotedString(contextItem.QUOTED_CONTEXT().getText().slice(0, -1).trim()),
                    isQuoted: true,
                    sourceInfo: {
                        file: this.currentFile,
    ...
    parseCodeLexeme(conceptText, parentCtx) {
        var _a;
        const concept = (0, parseCodeLexeme_1.parseCodeLexeme)(conceptText);
    ...
    parseNewElement(ctx) {
        ...
        if ((0, isEmpty_1.default)(card.max)) {
        ...
        if ((0, isEmpty_1.default)(ctx.STRING())) {
            ...
            if ((0, isEmpty_1.default)(ctx.STRING()[1]) && (0, isEmpty_1.default)(ctx.MULTILINE_STRING())) {
                // Default definition to the value of short
                addElementRule.definition = addElementRule.short;
            }
            else if (!(0, isEmpty_1.default)(ctx.STRING()[1])) {
    ...
    getPathWithContext(path, parentCtx, isPathRule = false, isInstanceRule = false) {
        const splitPath = path === '.' ? [path] : (0, common_1.splitOnPathPeriods)(path).filter(p => p);
        return this.getArrayPathWithContext(splitPath, parentCtx, isPathRule, isInstanceRule).join('.');
    }
    getArrayPathWithContext(pathArray, parentCtx, isPathRule = false, isInstanceRule = false) {
        return this.prependPathContext(pathArray, parentCtx, isPathRule, isInstanceRule);
    }
    visitPath(ctx) {
        return (ctx === null || ctx === void 0 ? void 0 : ctx.getText()) || '';
    }
    visitCaretPath(ctx) {
    ...
    visitFixedValueRule(ctx) {
        ...
        if ((_a = ctx.value()) === null || _a === void 0 ? void 0 : _a.NUMBER()) {
            assignmentRule.rawValue = ctx.value().NUMBER().getText();
        }
        if ((_b = ctx.value()) === null || _b === void 0 ? void 0 : _b.bool()) {
    ...
    visitConcept(ctx) {
        ...
        if (localCodePath.some(listedConcept => listedConcept.system)) {
            ...
            if (ctx.parentCtx instanceof FSHParser_1.default.RuleSetRuleContext &&
                codePart.system &&
                !concept.definition &&
    ...
    parseTargetType(ctx) {
        ...
        ctx.targetType().forEach(t => {
            if (t.referenceType()) {
                ...
                codeableReferences.forEach(r => orTypes.push({
    ...
    visitCaretValueRule(ctx) {
        ...
        caretValueRule.isInstance =
            ((_c = ctx.value()) === null || _c === void 0 ? void 0 : _c.name()) != null && !this.allAliases.has(ctx.value().name().getText());
    ...
    visitPathRule(ctx, isInstanceRule = false) {
        const pathRule = new rules_1.PathRule(this.getPathWithContext(this.visitPath(ctx.path()), ctx, true, isInstanceRule))
            .withLocation(this.extractStartStop(ctx))
            .withFile(this.currentFile);
        return pathRule;
    }
    ...
    parseRuleSetReference(ctx) {
        const ruleSetName = ctx.PARAM_RULESET_REFERENCE().getText().slice(0, -1).trim();
        const ruleSetParams = [...ctx.parameter(), ctx.lastParameter()].map(param => param.getText().slice(0, -1).trim());
        return { name: ruleSetName, params: ruleSetParams };
    }
    parseInsertRuleParams(ctx) {
            ...
            if (ruleSet) {
                ...
                if (ruleSet.parameters.length === insertRule.params.length) {
                    ...
                    if (!this.currentDoc.appliedRuleSets.has(ruleSetIdentifier)) {
                        ...
                        const appliedFsh = (0, MiniFSHImporter_1.applyRuleSetSubstitutions)(ruleSet, insertRule.params);
    ...
    parseGeneratedRuleSet(input, name, insertRule) {
        ...
        if (this.topLevelParse) {
            ...
            topLevelInfo = (0, FSHLogger_1.switchToSecretLogger)();
        }
        try {
        ...
        if (topLevelInfo) {
            ...
            const collectedMessages = (0, FSHLogger_1.restoreMainLogger)(topLevelInfo);
            this.topLevelParse = true;
     ...
     */
    prependPathContext(path, parentCtx, isPathRule, isInstanceRule) {
        try {
            ...
            if (contextIndex === 0) {
                ...
                if (this.pathContext.length > 0 &&
                    ...
                    FSHLogger_1.logger.warn('The previous line(s) use path rules to establish a context using soft indexing ' +
    ...
    extractMultilineString(mlStringCtx) {
        ...
        const minSpaces = (0, min_1.default)(lines.map(line => {
    ...
    extractNumberValue(numberCtx) {
        ...
        const [, wholePart, decimalPart, exponentPart] = (_a = numberString.match(numberSplitter)) !== null && _a !== void 0 ? _a : [];
        const exponentValue = exponentPart ? parseInt(exponentPart.slice(1)) : 0;
        if (decimalPart) {
            const decimalTrimmer = /\.(\d*[1-9])0*/;
            const [, trimmedDecimal] = (_b = decimalPart.match(decimalTrimmer)) !== null && _b !== void 0 ? _b : [];
            if (trimmedDecimal) {
                if (trimmedDecimal.length <= exponentValue) {
                    return (BigInt(`${wholePart}${trimmedDecimal}`) *
        ...
        const [, wholeZeroes] = (_c = wholePart.match(wholeZeroCatcher)) !== null && _c !== void 0 ? _c : [];
        const wholeZeroCount = (_d = wholeZeroes === null || wholeZeroes === void 0 ? void 0 : wholeZeroes.length) !== null && _d !== void 0 ? _d : 0;
