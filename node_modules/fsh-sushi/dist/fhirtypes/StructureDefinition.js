...
class StructureDefinition {
    ...
    addElement(element) {
        ...
        for (; i < this.elements.length; i++) {
            ...
            if (new RegExp(`^${(0, escapeRegExp_1.default)(currentId)}[.:/]`).test(element.id)) {
                lastMatchId = currentId;
            }
            else if (!new RegExp(`^${(0, escapeRegExp_1.default)(lastMatchId)}[./:]`).test(currentId) ||
                // If element is not a slice at this level, and the currentId is a slice, break to add children before slices
                (new RegExp(`^${(0, escapeRegExp_1.default)(lastMatchId)}[.]`).test(element.id) &&
                    new RegExp(`^${(0, escapeRegExp_1.default)(lastMatchId)}[:/]`).test(currentId))) {
                break;
            }
        }
