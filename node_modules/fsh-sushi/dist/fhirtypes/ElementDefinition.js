...
exports.ElementDefinition = exports.ElementDefinitionType = void 0;
...
const PROFILE_ELEMENT_EXTENSION = 'http://hl7.org/fhir/StructureDefinition/elementdefinition-profile-element';
class ElementDefinitionType {
    ...
    get code() {
        ...
        return (_c = (_b = fhirTypeExtension === null || fhirTypeExtension === void 0 ? void 0 : fhirTypeExtension.valueUrl) !== null && _b !== void 0 ? _b : fhirTypeExtension === null || fhirTypeExtension === void 0 ? void 0 : fhirTypeExtension.valueUri) !== null && _c !== void 0 ? _c : this._actualCode;
    }
    set code(c) {
        this._actualCode = c;
    }
    ...
    toJSON() {
        ...
        const elDefTypeClone = (0, lodash_1.cloneDeep)(this);
...
class ElementDefinition {
    ...
    validate() {
        ...
        if (this.slicing) {
            if (this.sliceName) {
                errors.push(new errors_1.ValidationError('An element with a slice name should not define its own slicing. Instead, append additional discriminators to the original slicing on the base element.', '', 'warn'));
            }
            errors.push(...this.validateSlicing(this.slicing));
        }
        return errors;
    }
    validateRequired(value, fshPath) {
        if (!value) {
            return new errors_1.ValidationError('Missing required value', fshPath);
        }
        return null;
    }
    validateIncludes(value, allowedValues, fshPath) {
        if (value && !allowedValues.includes(value)) {
            return new errors_1.ValidationError(`Invalid value: #${value}. Value must be selected from one of the following: ${allowedValues
                .map(v => `#${v}`)
                .join(', ')}`, fshPath);
        }
        return null;
    ...
    isPrimitive(fisher) {
        ...
        if (this.type == null && this.contentReference != null) {
            ...
            if (((_a = referencedElement === null || referencedElement === void 0 ? void 0 : referencedElement.type) === null || _a === void 0 ? void 0 : _a.length) === 1) {
                const typeSD = fisher.fishForFHIR(referencedElement.type[0].code, utils_1.Type.Type);
                return (typeSD === null || typeSD === void 0 ? void 0 : typeSD.kind) === 'primitive-type';
            }
        }
        if (((_b = this.type) === null || _b === void 0 ? void 0 : _b.length) === 1) {
            const typeSD = fisher.fishForFHIR(this.type[0].code, utils_1.Type.Type);
            return (typeSD === null || typeSD === void 0 ? void 0 : typeSD.kind) === 'primitive-type';
        }
        return false;
    }
    validateSlicing(slicing) {
        var _a;
        const validationErrors = [];
        validationErrors.push(this.validateRequired(slicing.rules, 'slicing.rules'));
        validationErrors.push(this.validateIncludes(slicing.rules, ALLOWED_SLICING_RULES, 'slicing.rules'));
        (_a = slicing.discriminator) === null || _a === void 0 ? void 0 : _a.forEach((d, i) => {
            const discriminatorPath = `slicing.discriminator[${i}]`;
            validationErrors.push(this.validateRequired(d.type, `${discriminatorPath}.type`));
            validationErrors.push(this.validateIncludes(d.type, ALLOWED_DISCRIMINATOR_TYPES, `${discriminatorPath}.type`));
    ...
    hasDiff() {
        ...
        return (PROPS_AND_UNDERPROPS.some(prop => {
            if (prop.endsWith('[x]')) {
            ...
            // @ts-ignore
            return prop && !(0, lodash_1.isEqual)(this[prop], original[prop]);
        }) ||
    ...
    calculateDiff() {
        ...
        for (let prop of PROPS_AND_UNDERPROPS) {
            ...
            // @ts-ignore
            if (prop && !(0, lodash_1.isEqual)(this[prop], original[prop])) {
                if (ADDITIVE_PROPS.includes(prop)) {
                    // @ts-ignore
                    diff[prop] = (0, lodash_1.differenceWith)(this[prop], original[prop], lodash_1.isEqual);
                    // @ts-ignore
                    if ((0, lodash_1.isEmpty)(diff[prop])) {
                        // @ts-ignore
                        delete diff[prop];
                    }
                }
                    ...
                    // @ts-ignore
                    diff[prop] = (0, lodash_1.cloneDeep)(this[prop]);
                }
            }
     ...
     * @param {Fishable} fisher - A fishable implementation for finding definitions and metadata
     */
    applyAddElementRule(rule, fisher) {
        ...
        if (!this.parent()) {
            throw new errors_1.CannotResolvePathError(rule.path);
        }
        ...
        const elementSD = fisher.fishForFHIR('Element', utils_1.Type.Type);
        ...
        elementSD.snapshot.element[0].constraint.forEach((c) => {
        ...
        this.applyFlags(rule.mustSupport, rule.summary, rule.modifier, rule.trialUse, rule.normative, rule.draft);
        // Since 'short' and 'definition' are plain string values, directly assign the values.
        if (!(0, lodash_1.isEmpty)(rule.short)) {
            this.short = rule.short;
        }
        if ((0, lodash_1.isEmpty)(rule.definition)) {
            if (!(0, lodash_1.isEmpty)(rule.short)) {
     ...
     * @private
     */
    initializeElementType(rule, fisher) {
        if (rule.types.length > 1 && !rule.path.endsWith('[x]')) {
            ...
            if (!rule.types.every(t => t.isReference) &&
                ...
                throw new errors_1.InvalidChoiceTypeRulePathError(rule);
            }
        }
        let refTypeCnt = 0;
        let canTypeCnt = 0;
        ...
        const finalTypes = (0, lodash_1.uniqWith)(initialTypes, lodash_1.isEqual);
        if (refTypeCnt > 0) {
        ...
        if (rule.types.length !== finalTypes.length + refCheckCnt + canCheckCnt + codableRefCheckCnt) {
            utils_1.logger.warn(`${rule.path} includes duplicate types. Duplicates have been ignored.`, rule.sourceInfo);
        }
        return finalTypes;
    }
    /**
     ...
     */
    getPreloadedSlices() {
        return this.getSlices().filter(slice => slice.min > 0 &&
            (Object.keys(slice).find(k => k.startsWith('fixed') || k.startsWith('pattern')) ||
                slice
                    .getAssignableDescendents()
                    .some((element) => Object.keys(element).find(k => k.startsWith('fixed') || k.startsWith('pattern')))));
    }
    /**
    ...
    constrainCardinality(min, max) {
        ...
        if (this.min != null && min < this.min) {
            throw new errors_1.ConstrainingCardinalityError(this.min, this.max, min, max);
        }
        ...
        if (this.slicing && !isUnbounded) {
            ...
            if (overMaxChildren.length > 0) {
                utils_1.logger.warn(`At least one slice of ${this.id} has a max greater than the overall element max. The max of the following slice(s) has been reduced to match the max of ${this.id}: ${overMaxChildren.join(',')}`);
            }
        }
        const connectedElements = this.findConnectedElements().filter(ce => !(ce.path === this.path && ce.id.startsWith(this.id)));
        ...
        connectedElements.forEach(ce => {
            ...
            if ((ce.max != null && ce.max !== '*' && min > parseInt(ce.max)) ||
                (ce.min != null && !isUnbounded && maxInt < ce.min)) {
                throw new errors_1.NarrowingRootCardinalityError(this.path, ce.id, min, max, ce.min, (_a = ce.max) !== null && _a !== void 0 ? _a : '*');
            }
        });
        ...
        if (slicedElement) {
            ...
            const sliceSiblings = this.structDef.elements.filter(el => this !== el &&
            ...
            const parentElement = parentSlice !== null && parentSlice !== void 0 ? parentSlice : slicedElement;
            // Check that parentElement max >= new sum of mins
            if (parentElement.max !== '*' && newParentMin > parseInt(parentElement.max)) {
                throw new errors_1.InvalidSumOfSliceMinsError(newParentMin, parentElement.max, parentElement.id);
            }
    ...
    findConnectedElements(postPath = '') {
        ...
        const connectedElements = slicesToUse
            ...
            .filter(e => e);
        if (this.parent()) {
            const [parentPath] = (0, common_1.splitOnPathPeriods)(this.id).slice(-1);
    ...
    findConnectedSliceElement(postPath = '') {
        const slicingRoot = this.slicedElement();
        if (slicingRoot) {
            return this.structDef.findElement(`${slicingRoot.id}${postPath}`);
        }
        else if (this.parent()) {
            return this.parent().findConnectedSliceElement(`.${this.path.split('.').slice(-1)[0]}${postPath}`);
        }
    }
    findParentSlice() {
        if (this.sliceName) {
    ...
    constrainType(rule, fisher, target) {
        ...
        if (targetTypes.some(t => t.code === 'CodeableReference') &&
            !targetTypes.some(t => t.code === 'Reference') &&
            rule.types.some(t => t.isReference)) {
            utils_1.logger.warn('The CodeableReference() keyword should be used to constrain references of a CodeableReference', rule.sourceInfo);
        }
        ...
        if (this.type.length === 1 &&
            ...
            ((_c = (_b = (_a = this.parent()) === null || _a === void 0 ? void 0 : _a.type) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.code) === 'CodeableReference') {
            utils_1.logger.error("Constraining references on a CodeableReference element's underlying .reference path is not allowed. Instead, constrain the references directly on the CodeableReference element.", rule.sourceInfo);
        }
        ...
        for (const type of types) {
            ...
            if ((type.isCanonical || type.isReference || type.isCodeableReference) &&
                type.type.indexOf('|') > -1) {
        ...
        const invalidReferenceTargets = logicalReferenceTargets.filter(typeMatch => typeMatch.metadata.canBeTarget === false);
        if (invalidReferenceTargets.length > 1) {
            const typeList = invalidReferenceTargets.map(info => info.typeName).join(', ');
            utils_1.logger.warn(`Referenced types ${typeList} do not specify that they can be the targets of a reference.`, rule.sourceInfo);
        }
        else if (invalidReferenceTargets.length === 1) {
            utils_1.logger.warn(`Referenced type ${invalidReferenceTargets[0].typeName} does not specify that it can be the target of a reference.`, rule.sourceInfo);
        }
        ...
        for (const type of this.type) {
            ...
            if (!typeMatches.has(type.code)) {
                newTypes.push((0, lodash_1.cloneDeep)(type));
                continue;
            }
        ...
        for (const newType of newTypes) {
            ...
            if (originalType && ((_d = originalType._profile) === null || _d === void 0 ? void 0 : _d.length)) {
                newType._profile = newType.profile.map(newProfile => {
                    var _a;
                    return (_a = originalType._profile[originalType.profile.indexOf(newProfile)]) !== null && _a !== void 0 ? _a : null;
                });
            ...
            if (originalType && ((_e = originalType._targetProfile) === null || _e === void 0 ? void 0 : _e.length)) {
                newType._targetProfile = newType.targetProfile.map(newProfile => {
                    var _a;
                    return ((_a = originalType._targetProfile[originalType.targetProfile.indexOf(newProfile)]) !== null && _a !== void 0 ? _a : null);
                });
        ...
        if (((_f = typeMatches.get('Extension')) === null || _f === void 0 ? void 0 : _f.length) > 0) {
            ...
            typeMatches.get('Extension').forEach(typeMatch => {
                const fullExtension = fisher.fishForFHIR(typeMatch.metadata.url, utils_1.Type.Extension);
                if (fullExtension) {
                    const isModifier = (0, common_1.isModifierExtension)(fullExtension);
                    if (isModifier && !isModifierPath) {
                        utils_1.logger.error(`Modifier extension ${typeMatch.metadata.name} used to constrain extension element. Modifier extensions should only be used with modifierExtension elements.`, rule.sourceInfo);
                    }
                    else if (!isModifier && isModifierPath) {
                        utils_1.logger.error(`Non-modifier extension ${typeMatch.metadata.name} used to constrain modifierExtension element. Non-modifier extensions should only be used with extension elements.`, rule.sourceInfo);
                    }
                }
            });
        }
    ...
    getTargetType(target, fisher) {
        ...
        let targetType;
        if (target) {
            const targetSD = fisher.fishForMetadata(target, utils_1.Type.Resource, utils_1.Type.Logical, utils_1.Type.Type, utils_1.Type.Profile, utils_1.Type.Extension);
            if (targetSD == null) {
            ...
            targetType = (0, lodash_1.cloneDeep)(this.type.find(t => {
                var _a, _b;
                return t.code === targetSD.id ||
                    ((_a = t.profile) === null || _a === void 0 ? void 0 : _a.includes(targetSD.url)) ||
                    ((_b = t.targetProfile) === null || _b === void 0 ? void 0 : _b.includes(targetSD.url));
            }));
            if (!targetType) {
    ...
    findTypeMatch(type, targetTypes, fisher) {
        ...
        for (const md of lineage) {
            if (type.isReference) {
                ...
                if (!matchedType) {
                    matchedType = targetTypes.find(t2 => t2.code === 'CodeableReference' &&
                        (t2.targetProfile == null || t2.targetProfile.includes(md.url)));
                }
            }
                ...
                matchedType = targetTypes.find(t2 => t2.code === 'canonical' &&
                    (t2.targetProfile == null || t2.targetProfile.includes(md.url)));
            }
                ...
                matchedType = targetTypes.find(t2 => {
                    var _a;
                    const matchesUnprofiledResource = t2.code === md.id && (0, lodash_1.isEmpty)(t2.profile);
                    const matchesProfile = this.getTypeLineage(md.sdType, fisher).some(ancestor => ancestor.sdType === t2.code) &&
                        ((_a = t2.profile) === null || _a === void 0 ? void 0 : _a.includes(md.url));
        ...
        if (!matchedType) {
            ...
            throw new errors_1.InvalidTypeError(typeString, targetTypes);
        }
        else if (specializationOfNonAbstractType) {
            throw new errors_1.NonAbstractParentOfSpecializationError(type.type, matchedType.code);
        }
        return {
            metadata: lineage[0],
        ...
        while (currentType != null) {
            ...
            if (result == null) {
                ...
                if (result && version != null && result.version != null && result.version != version) {
                    utils_1.logger.warn(`${type} is based on ${name} version ${version}, but SUSHI found version ${result.version}`);
                }
            }
            if (result) {
                results.push(result);
            }
            currentType = result === null || result === void 0 ? void 0 : result.parent;
        }
        return results;
    }
    /**
    ...
    applyProfiles(newType, targetType, matches) {
        ...
        for (const match of matches) {
            ...
            else if ((0, common_1.isReferenceType)(match.code) && !(0, common_1.isReferenceType)(match.metadata.sdType)) {
                matchedTargetProfiles.push(match.metadata.url);
            }
            else if (match.code === 'canonical' && match.metadata.sdType !== 'canonical') {
                matchedTargetProfiles.push(match.metadata.url);
            }
            else if (this.structDef.kind === 'logical' &&
        ...
        if (targetType) {
            if (!(0, lodash_1.isEmpty)(matchedTargetProfiles)) {
                ...
                if (targetIdx != null && targetIdx > -1) {
                    ...
                    newType.targetProfile = (_b = newType.profile) !== null && _b !== void 0 ? _b : [];
                    newType.targetProfile.push(...matchedTargetProfiles);
                }
            }
            if (!(0, lodash_1.isEmpty)(matchedProfiles)) {
                ...
                if (targetIdx != null && targetIdx > -1) {
                    ...
                    newType.profile = (_d = newType.profile) !== null && _d !== void 0 ? _d : [];
    ...
    findTypeIntersection(leftTypes, rightTypes, targetType, fisher) {
        ...
        leftTypes.forEach(left => {
            ...
            const matches = [];
            const typesToTry = ((_a = left.profile) === null || _a === void 0 ? void 0 : _a.length) ? left.profile : [left.code];
    ...
    applyTypeIntersection(type, targetType, matches) {
        ...
        for (const match of matches) {
            ...
            const typeString = (0, common_1.isReferenceType)(match.code) || match.code === 'canonical'
                ? match.code
        ...
        for (const [typeCode, currentMatches] of currentTypeMatches) {
            const newType = (0, lodash_1.cloneDeep)(type);
    ...
    applyFlags(mustSupport, summary, modifier, trialUse, normative, draft) {
        ...
        if (mustSupport === true) {
            if (this.structDef.derivation === 'specialization') {
                ...
                throw new errors_1.InvalidMustSupportError(this.structDef.name, this.id);
            }
     ...
     */
    bindToVS(vsURI, strength, ruleSourceInfo) {
        var _a, _b, _c, _d, _e, _f;
        // Check if this is a valid type to be bound against
        const validTypes = this.findTypesByCode('code', 'Coding', 'CodeableConcept', 'CodeableReference', 'Quantity', 'string', 'uri');
        if ((0, lodash_1.isEmpty)(validTypes)) {
        ...
        if (this.type.length === 1 &&
            ...
            ((_c = (_b = (_a = this.parent()) === null || _a === void 0 ? void 0 : _a.type) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.code) === 'CodeableReference') {
            utils_1.logger.error("Applying value set bindings to a CodeableReference element's underlying .concept path is not allowed. Instead, apply the binding directly to the CodeableReference element.", ruleSourceInfo);
        }
        ...
        if (((_d = listElement === null || listElement === void 0 ? void 0 : listElement.binding) === null || _d === void 0 ? void 0 : _d.valueSet) == vsURI &&
            strengths.indexOf(strength) < strengths.indexOf((_e = listElement === null || listElement === void 0 ? void 0 : listElement.binding) === null || _e === void 0 ? void 0 : _e.strength)) {
            throw new errors_1.BindingStrengthError((_f = listElement === null || listElement === void 0 ? void 0 : listElement.binding) === null || _f === void 0 ? void 0 : _f.strength, strength);
        }
        // Canonical URLs may include | to specify version: https://www.hl7.org/fhir/references.html#canonical
        if (!(0, valid_url_1.isUri)(vsURI.split('|')[0])) {
     ...
     */
    assignValue(value, exactly = false, fisher) {
        var _a, _b, _c, _d, _e, _f;
        let type;
        ...
        else if (value instanceof fshtypes_1.FshCanonical) {
            type = 'Canonical';
        }
        else if (value instanceof InstanceDefinition_1.InstanceDefinition) {
        ...
        switch (type) {
            case 'boolean':
                ...
                break;
            case 'number':
                this.assignNumber(value, exactly);
                break;
            case 'string':
                this.assignString(value, exactly);
                break;
            case 'Code':
                this.assignFshCode(value, exactly, fisher);
                break;
            case 'Quantity':
                value = value;
                ...
                if (actualType !== 'Quantity') {
                    const actualTypeSD = fisher === null || fisher === void 0 ? void 0 : fisher.fishForFHIR(actualType, utils_1.Type.Type);
                    if ((actualTypeSD === null || actualTypeSD === void 0 ? void 0 : actualTypeSD.baseDefinition) === 'http://hl7.org/fhir/StructureDefinition/Quantity') {
            ...
            case 'Reference':
                ...
                if (this.type[0].code === 'CodeableReference') {
                    throw new errors_1.AssignmentToCodeableReferenceError('reference', value, 'reference');
                }
                ...
                const referenceConstrainingElement = this.type[0].code === 'Reference' &&
                    ((_c = (_b = (_a = this.parent()) === null || _a === void 0 ? void 0 : _a.type) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.code) === 'CodeableReference'
                    ? this.parent()
                    : this;
                if (!referenceConstrainingElement.typeSatisfiesTargetProfile(value.sdType, value.sourceInfo, fisher)) {
                    throw new errors_1.InvalidTypeError(`Reference(${value.sdType})`, referenceConstrainingElement.type);
                }
            ...
            case 'Canonical':
                ...
                const canonicalDefinition = fisher.fishForMetadata(value.entityName, utils_1.Type.Resource, utils_1.Type.Logical, utils_1.Type.Type, utils_1.Type.Profile, utils_1.Type.Extension, utils_1.Type.ValueSet, utils_1.Type.CodeSystem, utils_1.Type.Instance);
                let canonicalUrl;
                if (!this.typeSatisfiesTargetProfile(canonicalDefinition === null || canonicalDefinition === void 0 ? void 0 : canonicalDefinition.resourceType, value.sourceInfo, fisher)) {
                    throw new errors_1.InvalidTypeError(`Canonical(${canonicalDefinition.resourceType})`, this.type);
                }
                if (canonicalDefinition === null || canonicalDefinition === void 0 ? void 0 : canonicalDefinition.url) {
                    canonicalUrl = canonicalDefinition.url;
                }
                else if ((canonicalDefinition === null || canonicalDefinition === void 0 ? void 0 : canonicalDefinition.id) && canonicalDefinition.instanceUsage === 'Inline') {
                    canonicalUrl = `#${canonicalDefinition.id}`;
                }
                else {
                    throw new errors_1.InvalidCanonicalUrlError(value.entityName);
                }
                if (value.version) {
            ...
            case 'InstanceDefinition':
                ...
                this.assignFHIRValue(stringVal, value.toJSON(), exactly, (_d = value._instanceMeta.sdType) !== null && _d !== void 0 ? _d : value.resourceType, fisher);
                break;
            default:
                type = (_f = (typeof value === 'object' && ((_e = value.constructor) === null || _e === void 0 ? void 0 : _e.name))) !== null && _f !== void 0 ? _f : type;
        ...
        parentSlices.forEach(parentSlice => {
            ...
            if (((_a = slicedElement.slicing.discriminator) === null || _a === void 0 ? void 0 : _a.some(d => d.path !== '$this' &&
     ...
     * @throws {MismatchedTypeError} when the value does not match the type of the ElementDefinition
     */
    assignFHIRValue(fshValue, fhirValue, exactly, type, fisher) {
        var _a, _b;
        ...
        const fixedX = `fixed${(0, lodash_1.upperFirst)(type)}`;
        const patternX = `pattern${(0, lodash_1.upperFirst)(type)}`;
        // Find any currently assigned values
        const currentElementValue = (_b = (_a = this[fixedX]) !== null && _a !== void 0 ? _a : this[patternX]) !== null && _b !== void 0 ? _b : this.assignedByAnyParent();
        ...
        if (currentElementValue != null &&
            ...
            throw new errors_1.ValueAlreadyAssignedError(fshValue, type, JSON.stringify(currentElementValue));
        }
    ...
    checkAssignedValueAgainstChildren(currentChild, fhirValue) {
        var _a;
        ...
        while (i < directChildren.length) {
            ...
            if (((_a = child.slicing) === null || _a === void 0 ? void 0 : _a.rules) === 'closed') {
                let numInvalid = 0;
                ...
                if (numInvalid >= child.getSlices().length) {
                    throw new errors_1.ValueConflictsWithClosedSlicingError(JSON.stringify(fhirValue));
                }
            }
    ...
    checkAssignedValueAgainstChild(child, fhirValue) {
        var _a;
        const childType = child.type[0].code;
        const fixedX = `fixed${(0, lodash_1.upperFirst)(childType)}`;
        const patternX = `pattern${(0, lodash_1.upperFirst)(childType)}`;
        const currentChildValue = (_a = child[fixedX]) !== null && _a !== void 0 ? _a : child[patternX];
        if (currentChildValue != null) {
            ...
            for (const pathPart of childPath) {
                newChildValue = (0, lodash_1.flatten)(newChildValue.map(value => {
                    return value === null || value === void 0 ? void 0 : value[pathPart];
                }));
            }
            newChildValue.forEach(value => {
                if (value != null) {
                    const childCompareFn = typeof value === 'object' ? lodash_1.isMatch : lodash_1.isEqual;
                    if (!childCompareFn(value, currentChildValue)) {
                        throw new errors_1.ValueAlreadyAssignedError(value, childType, JSON.stringify(currentChildValue));
                    }
                }
            });
        }
     ...
     */
    typeSatisfiesTargetProfile(sdType, sourceInfo, fisher) {
        ...
        if (sdType && this.type[0].targetProfile) {
            ...
            this.type[0].targetProfile.forEach(tp => {
                ...
                const tpType = (_a = (0, utils_1.fishForMetadataBestVersion)(fisher, tp, sourceInfo)) === null || _a === void 0 ? void 0 : _a.sdType;
                if (tpType) {
        ...
        if (parent == null) {
            ...
            if (assignedValue == null) {
                ...
                if (Array.isArray(parentValue)) {
                    ...
                    assignedValue = parentValue === null || parentValue === void 0 ? void 0 : parentValue[childIndex];
                }
            }
     ...
     * @throws {MismatchedTypeError} when the value does not match the type of the ElementDefinition
     */
    assignNumber(value, exactly = false) {
     ...
     */
    assignString(value, exactly = false) {
        const type = this.type[0].code;
        if (type === 'string' ||
            ...
            this[exactly ? 'fixedXhtml' : 'patternXhtml'] = (0, html_minifier_terser_1.minify)(value, {
        ...
        try {
            if (this.path.endsWith('.div')) {
                ...
                if (!startsWithDiv || !endsWithDiv) {
                    utils_1.logger.warn(`xhtml div elements should start and end with <div> tags for ${this.id}`);
                }
            }
     ...
     */
    checkAssignInlineInstance(value, fisher) {
        var _a, _b, _c, _d, _e;
        const inlineInstanceType = (_a = value.resourceType) !== null && _a !== void 0 ? _a : value._instanceMeta.sdType;
        const lineage = this.getTypeLineage(inlineInstanceType, fisher).map(metadata => metadata.sdType);
        if ((_b = this.type) === null || _b === void 0 ? void 0 : _b.some(t => lineage.includes(t.code))) {
            // capture original value to restore after assignment
            const originalKey = Object.keys(this).find(k => k.startsWith('pattern') || k.startsWith('fixed'));
            const originalValue = this[originalKey];
            // try assigning it to test for value conflicts
            const stringVal = JSON.stringify(value);
            this.assignFHIRValue(stringVal, value.toJSON(), true, (_c = value._instanceMeta.sdType) !== null && _c !== void 0 ? _c : value.resourceType, fisher);
            // if the assignment is successful, undo it and return the value
            const key = Object.keys(this).find(k => k.startsWith('pattern') || k.startsWith('fixed'));
            if (key != null) {
                delete this[key];
                // @ts-ignore
            ...
            throw new errors_1.MismatchedTypeError(inlineInstanceType, (_d = value._instanceMeta.name) !== null && _d !== void 0 ? _d : value.id, (_e = this.type) === null || _e === void 0 ? void 0 : _e.map(t => t.code).join(', '));
        }
    }
    /**
     ...
     */
    assignFshCode(code, exactly = false, fisher) {
        var _a, _b, _c, _d;
        const type = this.type[0].code;
        if (code.system) {
            const csURI = code.system.split('|')[0];
            const vsURI = (_b = (_a = (0, utils_1.fishForMetadataBestVersion)(fisher, code.system, code.sourceInfo, utils_1.Type.ValueSet)) === null || _a === void 0 ? void 0 : _a.url) !== null && _b !== void 0 ? _b : '';
            if (vsURI) {
                if (type === 'code' || type === 'string' || type === 'uri') {
                    utils_1.logger.warn(`The fully qualified code ${code.system}#${code.code} is invalid because the specified system is a ValueSet. ` +
                    ...
                    throw new errors_1.MismatchedBindingTypeError(code.system, this.path, 'CodeSystem');
                }
            }
            else if (!(0, valid_url_1.isUri)(csURI)) {
                if (type === 'code' || type === 'string' || type === 'uri') {
                    utils_1.logger.warn(`The fully qualified code ${code.system}#${code.code} is invalid because the specified system is not a URI. ` +
                        `Since ${this.path} is a ${type}, the system will not be used, but this issue should be corrected by ` +
                        `updating the system to refer to a proper CodeSystem or by specifying a code only (e.g., #${code.code}).`);
                }
                else {
        ...
        if (type === 'code' || type === 'string' || type === 'uri') {
            ...
            this.assignFHIRValue(code.toString(), code.toFHIRCodeableConcept(), exactly, 'CodeableConcept');
        }
        else if (type === 'Coding') {
            ...
            const existing = (_d = (_c = this.fixedQuantity) !== null && _c !== void 0 ? _c : this.patternQuantity) !== null && _d !== void 0 ? _d : this.assignedByAnyParent();
            const quantity = code.toFHIRQuantity();
            if ((existing === null || existing === void 0 ? void 0 : existing.value) != null) {
                quantity.value = existing.value;
            }
            if ((existing === null || existing === void 0 ? void 0 : existing.comparator) != null) {
            ...
            throw new errors_1.AssignmentToCodeableReferenceError('code', code, 'concept');
        }
        else {
            throw new errors_1.MismatchedTypeError('code', code, type);
        }
    }
    /**
     ...
     */
    isQuantityType(type, fisher) {
        if (type === 'Quantity') {
            return true;
        }
        const actualTypeSD = fisher === null || fisher === void 0 ? void 0 : fisher.fishForFHIR(type, utils_1.Type.Type);
        return (actualTypeSD === null || actualTypeSD === void 0 ? void 0 : actualTypeSD.baseDefinition) === 'http://hl7.org/fhir/StructureDefinition/Quantity';
    }
    /**
    ...
    hasSingleType() {
        var _a;
        const types = (_a = this.type) !== null && _a !== void 0 ? _a : [];
    ...
    applyMapping(identity, map, comment, language) {
        ...
        const mapping = Object.assign(Object.assign({ identity,
    ...
    unfold(fisher) {
        ...
        const proceedToUnfold = (((_a = this.type) === null || _a === void 0 ? void 0 : _a.length) === 1 && (!isChoiceElement || ((_b = this.type[0].profile) !== null && _b !== void 0 ? _b : []).length <= 1)) ||
            this.contentReference;
        if (proceedToUnfold) {
            let profileToUse;
            const availableProfiles = ((_c = this.type) === null || _c === void 0 ? void 0 : _c.length) === 1 ? this.type[0].profile : [];
            // if more than one profile is available, none of them can be used, since there's no way to decide.
            if ((availableProfiles === null || availableProfiles === void 0 ? void 0 : availableProfiles.length) > 1) {
                utils_1.logger.warn(`Multiple profiles present on element ${this.id}. Base element type will be used instead of any profiles.`);
            }
            else if ((availableProfiles === null || availableProfiles === void 0 ? void 0 : availableProfiles.length) === 1) {
            ...
            if (this.contentReference) {
                ...
                const profileJson = fisher.fishForFHIR(this.structDef.id, utils_1.Type.Profile);
            ...
            if (newElements.length === 0) {
                ...
                const type = profileToUse !== null && profileToUse !== void 0 ? profileToUse : this.type[0].code;
                // Fish on standard types, but if this is a logical model, also fish on logical models
                const typesToFish = [utils_1.Type.Resource, utils_1.Type.Type, utils_1.Type.Profile, utils_1.Type.Extension];
                if (((_d = this.structDef) === null || _d === void 0 ? void 0 : _d.kind) === 'logical') {
                    typesToFish.unshift(utils_1.Type.Logical);
                }
                ...
                let json = (0, utils_1.fishForFHIRBestVersion)(fisher, type, null, // no source info
                ...typesToFish);
                if (!json && profileToUse) {
                    utils_1.logger.warn(`SUSHI tried to find profile ${type} but could not find it and instead will try to use ${this.type[0].code}`);
                ...
                if (json) {
                    const def = StructureDefinition_1.StructureDefinition.fromJSON(json);
                    if (def.inProgress) {
                        utils_1.logger.debug(`Warning: Circular relationship detected between ${(_e = this.structDef) === null || _e === void 0 ? void 0 : _e.name} and ${def.name}. As a result, it is possible that the definition of ${(_f = this.structDef) === null || _f === void 0 ? void 0 : _f.name} may be based on incomplete components of ${def.name}.`);
                    }
     ...
     */
    cloneChildren(targetElement, recaptureSliceExtensions = true) {
        return targetElement === null || targetElement === void 0 ? void 0 : targetElement.children().map(e => {
            // Sometimes we want to avoid recapturing extensions, but if an element is not a slice
            // extension, we always capture it
            const shouldCaptureOriginal = recaptureSliceExtensions || e.sliceName == null || !e.path.endsWith('.extension');
     ...
     */
    hasProfileElementExtension(profileJson) {
        var _a, _b, _c;
        const elementName = this.getContentReferenceId();
        const elementType = (_b = (_a = profileJson.differential.element.find((element) => element.id === elementName)) === null || _a === void 0 ? void 0 : _a.type) === null || _b === void 0 ? void 0 : _b[0];
        // If the type property is not present in the differential, the profile-element extension is not present
        if (!elementType) {
            return false;
        }
        let profileCanonical, extensionUrl, targetElement;
        ...
        if (elementType.profile && elementType._profile) {
            const profileIndex = elementType._profile.findIndex((profile) => profile &&
                profile.extension.some((extension) => extension.hasOwnProperty('url') &&
                    extension.hasOwnProperty('valueString') &&
                    extension.url === PROFILE_ELEMENT_EXTENSION));
            if (profileIndex === -1) {
                return false;
            }
            const extensionIndex = (_c = elementType._profile[profileIndex].extension) === null || _c === void 0 ? void 0 : _c.findIndex((extensionObj) => extensionObj.hasOwnProperty('url') &&
        ...
        return (profileCanonical === this.structDef.url &&
     ...
     * @param {Fishable} fisher - A fishable implementation for finding definitions and metadata
     */
    unfoldChoiceElementTypes(fisher) {
        const allTypes = [];
        this.type.forEach(t => {
            var _a;
            if ((_a = t.profile) === null || _a === void 0 ? void 0 : _a.length) {
        ...
        const sharedAncestry = (0, lodash_1.intersectionWith)(...allTypeAncestry);
        if (sharedAncestry.length > 0) {
            const commonAncestor = StructureDefinition_1.StructureDefinition.fromJSON(fisher.fishForFHIR(sharedAncestry[0]));
            ...
            utils_1.logger.error(`Could not unfold choice element ${this.id}: choices have no common ancestor.`);
        }
        return [];
    }
    /**
    ...
    sliceIt(discriminatorType, discriminatorPath, ordered, rules) {
        if (this.sliceName) {
            ...
            utils_1.logger.warn(`${this.id} is a slice. Slices should not have slicing info added to them.`);
        }
    ...
    addSlice(name, type) {
        ...
        if (existingSlice) {
            throw new errors_1.DuplicateSliceError(this.structDef.name, this.id, name);
        }
        ...
        if (this.path.endsWith('[x]') &&
            this.type.length === 1 &&
            ((_b = (_a = this.slicing) === null || _a === void 0 ? void 0 : _a.discriminator) === null || _b === void 0 ? void 0 : _b[0].type) === 'type' &&
            ((_d = (_c = this.slicing) === null || _c === void 0 ? void 0 : _c.discriminator) === null || _d === void 0 ? void 0 : _d[0].path) === '$this') {
            slice.min = this.min;
        }
        else {
            slice.min = 0;
    ...
    isPartOfComplexExtension() {
        return (this.structDef.type === 'Extension' &&
            [this, ...this.getAllParents().slice(0, -1)].every(ed => { var _a; return ((_a = ed.type) === null || _a === void 0 ? void 0 : _a.length) === 1 && ed.type[0].code === 'Extension' && ed.sliceName != null; }));
    }
    /**
    ...
    clone(clearOriginal = true) {
        ...
        this.structDef = null;
        const clone = (0, lodash_1.cloneDeep)(this);
        ...
        for (let prop of PROPS_AND_UNDERPROPS) {
            ...
            if (prop && this[prop] !== undefined) {
                if (prop === 'type') {
                    ...
                    // @ts-ignore
                    j[prop] = (0, lodash_1.cloneDeep)(this[prop]);
                }
            }
        }
        return j;
        ...
        for (let prop of PROPS_AND_UNDERPROPS) {
            ...
            if (prop && json[prop] !== undefined) {
                if (prop === 'type') {
                    ...
                    // @ts-ignore
                    ed[prop] = (0, lodash_1.cloneDeep)(json[prop]);
                }
            }
        }
        if (captureOriginal) {
