...
const AbstractInstanceOfError_1 = require("../errors/AbstractInstanceOfError");
const MismatchedTypeError_1 = require("../errors/MismatchedTypeError");
...
class InstanceExporter {
     ...
     */
    warnOnInstanceOfCustomResource() {
        const instancesOfCustomResource = this.pkg.instances.filter(instance => this.pkg.resources.some(r => r.type === instance.resourceType));
    ...
    setAssignedValues(fshInstanceDef, instanceDef, instanceOfStructureDefinition) {
        var _a, _b;
        const manualSliceOrdering = (_b = (_a = this.tank.config.instanceOptions) === null || _a === void 0 ? void 0 : _a.manualSliceOrdering) !== null && _b !== void 0 ? _b : false;
        // The fshInstanceDef.rules list may contain insert rules, which will be expanded to AssignmentRules
        (0, common_1.applyInsertRules)(fshInstanceDef, this.tank);
        (0, utils_1.resolveSoftIndexing)(fshInstanceDef.rules, manualSliceOrdering);
        let rules = fshInstanceDef.rules.map(r => (0, lodash_1.cloneDeep)(r));
        // Normalize all rules to not use the optional [0] index
        rules.forEach(r => {
            r.path = r.path.replace(/\[0+\]/g, '');
        });
        rules = rules.map(r => r instanceof rules_1.PathRule ? r : (0, common_1.replaceReferences)(r, this.tank, this.fisher));
        // Convert strings in AssignmentRules to instances
        rules = rules.filter(r => {
            if (r instanceof rules_1.AssignmentRule && r.isInstance) {
        ...
        rules
            .filter(r => r instanceof rules_1.AssignmentRule)
            .forEach((r) => {
            var _a, _b, _c;
            ...
            if (r.path.endsWith('.resourceType') && typeof r.value === 'string') {
                inlineResourcePaths.push({
                    ...
                    path: (0, common_1.splitOnPathPeriods)(r.path).slice(0, -1).join('.'),
                    instanceOf: r.value
                });
            }
        });
        ...
        rules.forEach(rule => {
            ...
            const doRuleValidation = (value) => {
                const validatedRule = instanceOfStructureDefinition.validateValueAtPath(rule.path, value, this.fisher, inlineResourceTypes, rule.sourceInfo, manualSliceOrdering);
                ...
                if (validatedRule.pathParts.some(part => part.base.endsWith('[x]'))) {
                    utils_1.logger.error(`Unable to assign value at ${rule.path}: choice elements on an instance must use a specific type`, rule.sourceInfo);
                }
            ...
            try {
                ...
                matchingInlineResourcePaths.forEach(match => {
                    inlineResourceTypes[(0, common_1.splitOnPathPeriods)(match.path).length - 1] = match.instanceOf;
                });
                ...
                if (rule.path !== updatedPath) {
                    utils_1.logger.warn(`When assigning values on instances, use the choice element's type. Rule path changed from '${rule.path}' to '${updatedPath}'.`, rule.sourceInfo);
                    rule.path = updatedPath;
                }
                doRuleValidation(rule instanceof rules_1.AssignmentRule ? rule.value : null);
            }
            catch (originalErr) {
                ...
                if (rule instanceof rules_1.AssignmentRule &&
                    originalErr instanceof MismatchedTypeError_1.MismatchedTypeError &&
                    ...
                    if (instanceToAssign == null) {
                        ...
                        try {
                            ...
                            if (instanceErr instanceof MismatchedTypeError_1.MismatchedTypeError) {
                                utils_1.logger.error(originalErr.message, rule.sourceInfo);
                            }
                            else {
        ...
        const paths = [
            '',
            ...rules.map(rule => { var _a, _b; return (0, utils_1.assembleFSHPath)((_b = (_a = ruleMap.get(rule.path)) === null || _a === void 0 ? void 0 : _a.pathParts) !== null && _b !== void 0 ? _b : []).replace(/\[0\]/g, ''); })
        ];
        ...
        if (manualSliceOrdering) {
            knownSlices = (0, common_1.createUsefulSlices)(instanceDef, instanceOfStructureDefinition, ruleMap, this.fisher);
        }
        else {
            // Don't create slices, just determine what will be created later
            knownSlices = (0, common_1.determineKnownSlices)(instanceOfStructureDefinition, ruleMap, this.fisher);
        }
        (0, common_1.setImpliedPropertiesOnInstance)(instanceDef, instanceOfStructureDefinition, paths, inlineResourcePaths.map(p => p.path), this.fisher, knownSlices, manualSliceOrdering);
        const ruleInstance = (0, lodash_1.cloneDeep)(instanceDef);
        ruleMap.forEach(rule => {
            var _a, _b;
            (0, common_1.setPropertyOnInstance)(ruleInstance, rule.pathParts, rule.assignedValue, this.fisher);
            // was an instance of an extension used correctly with respect to modifiers?
            if ((0, common_1.isExtension)(rule.pathParts[rule.pathParts.length - 1].base) &&
                typeof rule.assignedValue === 'object') {
                const extension = this.fisher.fishForFHIR(rule.assignedValue.url, utils_1.Type.Extension);
                if (extension) {
                    ...
                    if (isModifier && pathBase === 'extension') {
                        utils_1.logger.error(`Instance of modifier extension ${(_a = extension.name) !== null && _a !== void 0 ? _a : extension.id} assigned to extension path. Modifier extensions should only be assigned to modifierExtension paths.`, rule.sourceInfo);
                    }
                    else if (!isModifier && pathBase === 'modifierExtension') {
                        utils_1.logger.error(`Instance of non-modifier extension ${(_b = extension.name) !== null && _b !== void 0 ? _b : extension.id} assigned to modifierExtension path. Non-modifier extensions should only be assigned to extension paths.`, rule.sourceInfo);
                    }
                }
            }
            ...
            rule.pathParts.forEach(pathPart => {
                ...
                if ((0, common_1.isExtension)(pathPart.base)) {
                    const sliceName = (0, common_1.getSliceName)(pathPart);
                    if (sliceName) {
                        const extension = this.fisher.fishForFHIR(sliceName, utils_1.Type.Extension);
                        if (extension) {
                            const isModifier = (0, common_1.isModifierExtension)(extension);
                            if (isModifier && pathPart.base === 'extension') {
                                utils_1.logger.error(`Modifier extension ${(_a = extension.name) !== null && _a !== void 0 ? _a : extension.id} used on extension element. Modifier extensions should only be used with modifierExtension elements.`, rule.sourceInfo);
                            }
                            else if (!isModifier && pathPart.base === 'modifierExtension') {
                                utils_1.logger.error(`Non-modifier extension ${(_b = extension.name) !== null && _b !== void 0 ? _b : extension.id} used on modifierExtension element. Non-modifier extensions should only be used with extension elements.`, rule.sourceInfo);
                            }
                        }
                    }
                }
        ...
        instanceDef = (0, lodash_1.merge)(instanceDef, ruleInstance);
        return instanceDef;
    }
    /**
     ...
     */
    validateRequiredChildElements(instance, element, fshDefinition, parentPrimitive) {
        ...
        children.forEach(c => {
            ...
            let instanceChild;
            let instanceChildPrimitive;
            let isChildTypePrimitive;
            ...
            if (instanceChild == null && childPathEnd.endsWith('[x]')) {
                ...
                for (const choiceSlice of choiceSlices) {
                    ...
                    instanceChild = (_a = instance[`_${choiceSlice.sliceName}`]) !== null && _a !== void 0 ? _a : instance[choiceSlice.sliceName];
                    const splitChoicePath = (0, common_1.splitOnPathPeriods)(choiceSlice.id);
                ...
                if (instanceChild == null) {
                    const namelessChoiceSlices = (_c = (_b = element
                        .findConnectedSliceElement()) === null || _b === void 0 ? void 0 : _b.children(true).filter(c => c.path === child.path && c.sliceName)) !== null && _c !== void 0 ? _c : [];
                    for (const choiceSlice of namelessChoiceSlices) {
                        ...
                        if (instanceChild != null) {
                            ...
                            utils_1.logger.warn(`Element ${child.id} has its cardinality satisfied by a rule that does not include the slice name. Use slice names in rule paths when possible.`);
                            child = choiceSlice;
                            break;
                        }
                    }
                ...
                if (instanceChild == null) {
                    for (const type of child.type) {
                        const name = childPathEnd.replace(/\[x\]$/, (0, lodash_1.upperFirst)(type.code));
                        instanceChild = (_d = instance[`_${name}`]) !== null && _d !== void 0 ? _d : instance[name];
                        if (instanceChild != null) {
                            break;
                        }
                    }
            ...
            if (Array.isArray(instanceChild)) {
                ...
                instanceChild = instanceChild.filter((arrayEl, idx) => {
                    var _a;
                    if (!child.sliceName ||
                        (arrayEl === null || arrayEl === void 0 ? void 0 : arrayEl._sliceName) === child.sliceName ||
                        ((_a = arrayEl === null || arrayEl === void 0 ? void 0 : arrayEl._sliceName) === null || _a === void 0 ? void 0 : _a.toString().startsWith(`${child.sliceName}/`))) {
                ...
                if (isChildTypePrimitive) {
                    instanceChildPrimitive = (0, lodash_1.at)(instanceChildPrimitive, matchingIndices);
                }
                instanceChild.forEach((arrayEl, idx) => {
                    if (arrayEl != null && !(0, lodash_1.isEmpty)(arrayEl)) {
                        this.validateRequiredChildElements(arrayEl, child, fshDefinition, isChildTypePrimitive ? instanceChildPrimitive === null || instanceChildPrimitive === void 0 ? void 0 : instanceChildPrimitive[idx] : undefined);
                    }
                });
            }
            else if (instanceChild != null) {
                this.validateRequiredChildElements(instanceChild, child, fshDefinition, instanceChildPrimitive);
            }
            // Log an error if:
    ...
    shouldSetMetaProfile(instanceDef, instanceOfStructureDefinition) {
        var _a;
        const configSetMeta = (_a = this.tank.config.instanceOptions) === null || _a === void 0 ? void 0 : _a.setMetaProfile;
        const isInline = instanceDef._instanceMeta.usage === 'Inline';
        if (configSetMeta === 'never' ||
        ...
        return (instanceOfStructureDefinition.derivation === 'constraint' &&
            instanceOfStructureDefinition.elements.some(el => {
                var _a, _b;
                return el.path === `${instanceOfStructureDefinition.pathType}.meta` &&
                    ((_a = el.type) === null || _a === void 0 ? void 0 : _a[0].code) === 'Meta' &&
                    el.max === '1' &&
                    (((_b = el.base) === null || _b === void 0 ? void 0 : _b.max) == null || el.base.max === '1');
            }));
    }
    shouldSetId(instanceDef, instanceOfStructureDefinition) {
        var _a;
        if (((_a = this.tank.config.instanceOptions) === null || _a === void 0 ? void 0 : _a.setId) === 'standalone-only' &&
        ...
        return instanceOfStructureDefinition.elements.some(el => {
            var _a, _b;
            return el.path === `${instanceOfStructureDefinition.pathType}.id` &&
                ['string', 'id'].includes((_a = el.type) === null || _a === void 0 ? void 0 : _a[0].code) &&
                el.max === '1' &&
                (((_b = el.base) === null || _b === void 0 ? void 0 : _b.max) == null || el.base.max === '1');
        });
    }
     ...
     * slice.
     */
    checkForNamelessSlices(fshDef, instanceDef, instanceOfStructureDefinition) {
        const nonChoiceSlicedElements = instanceOfStructureDefinition.elements.filter(e => e.slicing && !e.path.endsWith('[x]'));
        for (const slicingEl of nonChoiceSlicedElements) {
            ...
            const { values } = (0, utils_1.collectValuesAtElementIdOrPath)(slicingEl.id, instanceDef);
            ...
            if (unnamedValues.length) {
                ...
                const namedValues = values
                    .filter(v => (v === null || v === void 0 ? void 0 : v._sliceName) != null)
                    .map(v => {
                    const cleanValue = (0, lodash_1.cloneDeep)(v);
                ...
                if (namedValues.length) {
                    for (const value of unnamedValues) {
                        ...
                        const matchedNamedValues = namedValues.filter(nv => (0, lodash_1.isMatch)(value, nv.value));
                        if (matchedNamedValues.length) {
                            ...
                            if (matchedNamedValues.every(nm => (0, lodash_1.isEqual)(value, nm.value))) {
                                utils_1.logger.warn(`${instanceDef._instanceMeta.name} has an array item that is exactly the same as a required slice, but does not ` +
                                    ...
                                    `  Value: ${valueString}}`, fshDef.sourceInfo);
                            }
                            else {
                                utils_1.logger.warn(`${instanceDef._instanceMeta.name} has an array item that matches a required slice, but does not use the slice name ` +
        ...
        if (result == null) {
            ...
            if (fshDefinition) {
                ...
                if (fishedFHIR && fishedFHIR.resourceType) {
                    result = fhirtypes_1.InstanceDefinition.fromJSON(fishedFHIR);
                }
            }
        }
        return result;
    ...
    applyInsertRules() {
        ...
        for (const instance of instances) {
            (0, common_1.applyInsertRules)(instance, this.tank);
        }
    }
    exportInstance(fshDefinition) {
        ...
        let isResource = true;
        const json = this.fisher.fishForFHIR(fshDefinition.instanceOf, utils_1.Type.Resource, utils_1.Type.Profile, utils_1.Type.Extension, utils_1.Type.Type, utils_1.Type.Logical);
        if (!json) {
        ...
        if (json.kind !== 'resource' && json.kind !== 'logical') {
            ...
            if (fshDefinition.usage !== 'Inline') {
                utils_1.logger.warn(`Instance ${fshDefinition.name} is not an instance of a resource or logical model, so it should only be used inline on other instances, and it will not be exported to a standalone file. Specify "Usage: #inline" to remove this warning.`, fshDefinition.sourceInfo);
        ...
        if ((ancestor === null || ancestor === void 0 ? void 0 : ancestor.abstract) === true) {
            throw new AbstractInstanceOfError_1.AbstractInstanceOfError(fshDefinition.name, ancestor.name, fshDefinition.sourceInfo);
        }
        ...
        if (fshDefinition.title == '') {
            utils_1.logger.warn(`Instance ${fshDefinition.name} has a title field that should not be empty.`);
        }
        if (fshDefinition.description == '') {
            utils_1.logger.warn(`Instance ${fshDefinition.name} has a description field that should not be empty.`);
        }
        if (fshDefinition.title) {
        ...
        if (fshDefinition.usage) {
            ...
            if (fshDefinition.usage === 'Definition') {
                if (instanceOfStructureDefinition.elements.some(element => element.id === `${instanceOfStructureDefinition.pathType}.url`)) {
                    instanceDef.url = `${this.tank.config.canonical}/${instanceOfStructureDefinition.pathType}/${fshDefinition.id}`;
                }
                if (fshDefinition.title &&
                    instanceOfStructureDefinition.elements.some(element => element.id === `${instanceOfStructureDefinition.pathType}.title`)) {
                    instanceDef.title = fshDefinition.title;
                }
                if (fshDefinition.description &&
                    instanceOfStructureDefinition.elements.some(element => element.id === `${instanceOfStructureDefinition.pathType}.description`)) {
        ...
        (0, common_1.cleanResource)(instanceDef);
        ...
        if ((_c = instanceDef.meta) === null || _c === void 0 ? void 0 : _c.profile)
            instanceDef.meta.profile = (0, lodash_1.uniq)(instanceDef.meta.profile);
        ...
        if (instanceDef._instanceMeta.usage !== 'Inline' &&
            this.pkg.instances.some(instance => {
                var _a, _b;
                return instance._instanceMeta.usage !== 'Inline' &&
                    instanceDef.resourceType === instance.resourceType &&
                    ((_a = instanceDef.id) !== null && _a !== void 0 ? _a : instanceDef._instanceMeta.name) ===
                        ((_b = instance.id) !== null && _b !== void 0 ? _b : instance._instanceMeta.name) &&
                    instanceDef !== instance;
            })) {
            utils_1.logger.error(`Multiple instances of type ${instanceDef.resourceType} with id ${(_d = instanceDef.id) !== null && _d !== void 0 ? _d : instanceDef._instanceMeta.name}. Each non-inline instance of a given type must have a unique id.`, fshDefinition.sourceInfo);
        }
        return instanceDef;
    }
    /**
